...
const CALIF_UI = {
  ...
  CONDICIONADO: { cls: 'revisar', icon: AlertTriangle, color: '#d97706' },
  'NO APTO': { cls: 'rechazo', icon: XCircle, color: '#e2132b' },
}
...
function PreEvaluacion() {
  ...
  const ui = res ? (CALIF_UI[res.calificacion] || CALIF_UI.CONDICIONADO) : null
  const Icon = ui?.icon
    ...
    <Card title="Pre-evaluación de capacidad de pago" icon={Gauge}>
      ...
      {res && (
        <div className={`cm-result ${ui.cls}`} style={{ marginTop: 18 }}>
          ...
          <div style={{ flex: 1 }}>
            <h3>{res.calificacion === 'NO APTO' ? 'No apto' : res.calificacion === 'APTO' ? 'Apto' : 'Condicionado'}</h3>
            <p>{res.motivo}</p>
